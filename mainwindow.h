...
public:
    ...
    // 重写虚函数（需要实现的功能） 通过参数QPaintEvent（绘图事件）实现
...
private:
    ...
    QByteArray recBuf;                          // 串口接收缓存区 QByteArray一个字节数组，不关心数据的内容 非常适合处理二进制数据和文本数据的混合
    void xFrameDataFilter(QByteArray *str, float value[]);
    void feature_extraction();                  // 特征值提取
    void set_measurement_value(const QString &regexPattern, QTextEdit *textEdit, const QString &recStr);//显示特征值
