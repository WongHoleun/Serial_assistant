...
public:
    ...
    void addDataToPlotData(float value[]);
...
//    void saveCurvesToCSV(const QString &fileName);
...
private:
    ...
    struct CurveData {
        QVector<float> curve1;
        QVector<float> curve2;
        QVector<float> curve3;
        QVector<float> curve4;
    };
    CurveData plotData; // 创建一个用于保存绘图数据的容器
};
